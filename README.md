...
Clone or download the project files (including server.js, src/App.jsx, src/App.css, vite.config.js, and package.json).
In the project directory, run:textnpm installThis installs all dependencies (React, Socket.IO, Express, Chart.js, etc.).

How to Start Locally (Development Mode)

Run both the frontend (Vite on port 5173) and backend (Node.js on port 3000) simultaneously:textnpm start
...
Enter a name to join. The first player is automatically the admin (marked as "(Admin)" in the player list).
As admin, click "Start Voting" to begin a round. Others can vote, but only admin controls start/reset.
...
Build the React app:textnpm run build
Creates a dist/ folder with optimized static files.

Start the backend server (which serves the frontend):textnpm run server
Access locally at http://localhost:3000.

Join as before: First player is admin.

Who is the Admin?

The first player to join (by entering their name) is designated as the admin.
Admin privileges:
...
Ensure the app is running in production mode (see above: npm run build then npm run server).
Download and install ngrok. Sign up for a free account if prompted.
In a new terminal, start ngrok to tunnel port 3000:textngrok http 3000
Ngrok output shows a public URL, e.g., https://random-subdomain.ngrok-free.app.
Copy the HTTPS forwarding URL.

Share the ngrok URL with others. They access it in their browsers to join the same session.
You (or others) join via the ngrok URL: Enter name, first joiner is admin.
